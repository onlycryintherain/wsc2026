...
| 2026-08-25 | pending | **placement별 stress density 자동 전환** — 1/1/1 shared run은 P95 첫 감지·노드 부팅 전 누적 손실로 2분 29.5/40(user42.75%, stress42.86%, avg2.5)이어서 사용자 제안의 작은 Pod 다중 prewarm을 반영. SHARED는 user2/product2/stress2를 Managed 1대에 배치하고 stress `350m@90%`(control315m), ISOLATED 전환 JSON Patch에서 selector와 함께 `600m@55%`(control330m)로 원자 변경해 전용 노드당 2 Pod를 보장. 실제 Managed 1대에 Pod 2/2/2·Karpenter0·Pending0 수렴, PS7/PS5 self-test 32/32 통과. |
| 2026-08-25 | pending | **placement selector 원자 교체** — merge patch가 stress의 기존 `karpenter.sh/nodepool=stress`와 새 `eks.amazonaws.com/nodegroup=...`를 동시에 남겨 Unschedulable을 만든 실측을 반영. 모든 MANAGED/ELASTIC/ISOLATED 전환을 JSON Patch의 nodeSelector 전체 add/remove로 교체하고, selector key가 정확히 하나인지 상태 판정에 포함. 실제 모순 selector 제거 후 user/product/stress Managed 1대 Ready 수렴, PS7/PS5 self-test 31/31 통과. |
| 2026-08-25 | pending | **P95 선행 탄력 확장** — 1노드 full-check run-1787612678에서 CPU metrics가 0~1%로 지연되는 동안 user P95 366ms·product 516ms·stress 이후 6500ms까지 먼저 악화되고, 약 36초 뒤 CPU/HPA pressure에서야 세 placement가 분리된 실측 반영. 각 SLO의 75%인 user/product 150ms, stress 600ms를 공통 pressure 신호에 추가해 access log 악화를 본 첫 decision window에서 foreground ELASTIC/stress ISOLATED와 pressure min을 함께 적용. PS7/PS5 self-test 31/31 통과. |
| 2026-08-25 | pending | **세 앱 1노드 병합 완성** — stress만 Managed selector를 써도 이전 run의 user/product가 default Karpenter에 남아 PDB로 노드 회수가 막힌 실측을 반영. Prepare/idle은 user/product/stress 모두 런타임 발견 Managed NodeGroup에 고정하고, user/product는 Pending/HOT/HPA pressure 시 selector를 제거해 default NodePool로 elastic rollout, 5분 유휴 시 Managed로 복귀하도록 추가. 실제 세 앱 Pod가 Managed 한 대에 1/1/1, Karpenter default/stress 0, ReadyNode1로 수렴. PS7/PS5 self-test 30/30 통과. |
| 2026-08-25 | pending | **동적 격리 선행조건 교정** — full-check run-1787612115에서 shared stress min을 TRAFFIC_WARM만으로 1→4로 먼저 올려 Managed 노드 Pending이 생기고, CPU metrics 지연 중 P95 2179ms인데도 격리가 늦는 경계 확인. 저부하 warm stress는 1로 유지하고 최근 30초 `P95≥600ms`, Pending, HOT/HPA pressure 중 하나면 HPA 증설보다 placement 격리를 먼저 수행하도록 변경. PS7/PS5 self-test 27/27 통과. |
| 2026-08-25 | pending | **observer 동적 stress 격리 상태 머신** — 사용자 목표에 맞춰 idle/시작 상태를 user1·product1·stress1 및 Managed NodeGroup 1대로 병합하고 HPA max 20/20/8은 개방. Managed NodeGroup 이름은 노드 label에서 런타임 발견해 shared selector로 사용하며, `HOT_POD`/`HPA_PRESSURE`/`HPA_MAX_PRESSURE`에서 stress 전용 NodePool로 `maxUnavailable=0` rolling 격리, 5분 연속 유휴에서만 Managed로 병합한다. pressure placement를 HPA min 변경보다 먼저 적용하고, stress 600m은 현재 CPU-heavy 바이너리에서 노드당 2 Pod 과밀 방지값으로 유지. 실제 Pod 1/1/1·ReadyNode1·stress/default Karpenter0 수렴, PS7/PS5 self-test 26/26 통과. |
| 2026-08-25 | pending | **scoring 시작 직전 backlog 원자 제거** — 유효하지 않은 full-check run-1787611222에서 준비 완료 후 수동 시작까지 약 2분 사이 외부 트래픽이 재유입돼 stress 한 Pod가 1.5코어 backlog, 첫 분 stress 0%·비용 게이트 off가 된 경합 확인. `-PrepareForLoad` stress restart 동안만 Deployment 전략을 `maxSurge=0/maxUnavailable=100%`로 전환해 기존 2개 노드에서 빠르게 전량 교체하고, finally에서 `maxSurge=1/maxUnavailable=0`으로 복구하도록 변경. |
| 2026-08-25 | pending | **저비용 idle과 scoring prewarm 분리** — `600m` stress baseline 2 Pod 후보(run-1787610123)가 첫 1분 stress100%·2노드였으나 연결 편중과 scale-up 지연으로 3분 stress55.17%·35/40·avg2.67까지 하락. 평상시 floor2는 유지하고 workflow `-PrepareForLoad`에서만 stress4를 2개 노드에 2개씩 prewarm(total3 node), 트래픽 중 warm4/pressure6/max8로 변경. 준비 시 이전 run이 남긴 유휴 2.9코어 CPU backlog를 stress rollout restart로 제거하고 정확한 Node3/Ready3/Pending0을 확인. `full-check`(15분: baseline3→spike1 4→spike2 5→cool3)를 `60분`보다 빠른 전체 단계 회귀 프로필로 선택. PS7/PS5 observer 23/23·workflow 3/3 통과. |
| 2026-08-25 | pending | **observer stress 과밀 방지 2-Pod/Node 프로필** — 공식 0.5x 재시험(run-1787609593)에서 `400m` stress 4개가 t3.medium 한 대에 배치된 뒤 실사용 2.6~3코어·P95 17.3초 backlog가 발생해 4분 36.5/40(stress 75.86%, avg2.25)로 하락한 실측 반영. stress를 `600m@55%`(absolute 330m), baseline/warm 2, pressure 4, max 8로 조정해 allocatable 1930m에서 daemon overhead 포함 노드당 정확히 2 Pod만 허용. 준비 완료 조건은 terminating 노드까지 사라진 전체 Node=2·Ready=2·Pending=0으로 강화. PS7/PS5 self-test 22/22 및 live 2-node rollout/HPA 검증. |
| 2026-08-25 | pending | **observer stress 1노드 밀집 수렴 보장** — Deployment에 남아 있던 hostname topology spread가 baseline 4 Pod를 stress 3노드로 분산해 비용을 높이고, Karpenter는 동일 가격 노드 재배치를 거부한 원인을 실측. observer가 spread를 제거하고 preferred podAffinity로 같은 hostname을 우선하도록 변경해 stress 4/4가 t3.medium 1대에 배치됨을 확인. `-PrepareForLoad` 동안만 disruption `100%/0s`로 빈 노드 회수를 허용하고 finally에서 `10%/1m` 복구, 실제 총 Ready 노드 4→2 수렴 및 PS7/PS5 self-test 22/22 통과. |
...
| 날짜 | 커밋 | 변경 요약 |
| 2026-08-25 | pending | **observe-live 동적 2노드 shared→stress isolate 재설계** — 기본 Managed+default 2노드에서 user/product 120m@19/17%, stress shared 280m·400Mi/4 Pod로 시작하고 P95 압력 시 foreground 10/6 및 stress 500m·640Mi/8..12 격리, default/stress ceiling 각 8 CPU를 수요 기반 사용. selector 원자 교체·활성 중 min 하향 금지·배치별 request·5초 관측·2노드 Prepare 검증 추가. full-check 실측 baseline 40/40 및 2분 38/40, self-test 34/34. 이후 계정 전체 외부 삭제(EKS nodegroup/IAM/S3/ECR/RDS Proxy 등)로 full 15분 검증 중단. |
| 2026-08-25 | pending | **stress 1 Pod/Node 성능 확장** — 격리 후 user는 100%로 회복했으나 60분 run stress가 6분63.4%→7분55.2%로 하락하고 4개 stress 노드가 77~103% CPU인 실측을 근거로, stress request/target을 `600m@55%`→`1000m@33%`로 변경해 absolute control point 330m를 보존. max8 Pod를 노드당 1개로 배치하도록 stress NodePool ceiling을 8대로 확대해 비용0 상태에서 stress 성능 점수 회복 우선 |
| 2026-08-25 | pending | **앱별 성능 우선 observer + 60분 workflow 교정** — shared stress run(평균4.67노드, user43.2%/stress28.46%)과 isolated prewarm run(평균5.25노드, user91.4%/stress65.3%) 비교를 근거로 stress 전용 placement를 세션 동안 고정, user `200m@12%` control point·warm12/max48, product warm2/max20, stress warm4/max8 적용. NodePool ceiling을 부하 전 선개방하고 workflow는 observer warm Ready 후 부하 시작, 성능 게이트는 15분 grace+opt-in으로 변경. observe self-test16/16, workflow3/3, PS7/Windows PowerShell/live WhatIf 통과 |
| 2026-08-25 | pending | **observe-live 성능 우선 시작 프로필** — 17분 채점 로그에서 user가 spike 후 약 5분간 1~5 Pod/1~2 Node에 묶여 P95 12~70초로 붕괴하고 product만 과확장된 현상을 근거로, 실행 즉시 user `200m` request·HPA `12/48@12%`, product `2/20@29%`, stress 전용 NodePool·`4/12@55%`를 적용. user control point(약 24m)는 보존하고 stress 간섭 및 70m 과밀 배치를 제거하며 self-test 15/15 + live WhatIf 통과 |
| 2026-08-25 | pending | HPA 즉시 개방 후 총 5노드/Ready user40·product23·stress5에서도 live P95 305/253/3634ms로 SLO 미달, 노드 CPU 최대62% 확인. 비용 추가 허용에 따라 default ceiling을 6대(12 CPU), user/product Max 80/60, active warm min 8/8/8로 상향하고 pressure 시 measured desired가 낮아도 warm floor까지 즉시 prewarm하도록 변경. stress는 기존 전용 최대4대/Max12 유지. |
| 2026-08-25 | pending | 단일 노드 복귀 후 실행된 observer가 HPA Max를 1→2→4 순차로 앱 하나씩 30초 간격 개방해 user/product/stress가 각각 5/8/4 ceiling, CPU 190/70/152%에서 초기 가용성·성능 점수를 잃은 원인 수정. HPA pressure 검증 시 Max를 user/product 40·stress 12로 즉시 열고, 같은 판단 주기에 NodePool과 세 앱 HPA를 모두 적용하도록 성능 우선 제어로 변경. max/limit 자체는 Pod/EC2를 선생성하지 않으며 실제 수요만 확장. |
| 2026-08-25 | pending | 단일 시작 노드 실검증에서 stress preferred affinity만으로도 Karpenter가 전용 노드를 즉시 생성함을 확인해 affinity 제거. stress Pod는 무부하/가용 shared 용량에서는 Managed node에 배치하고, 실제 Unschedulable 시에만 toleration + stress NodePool weight 100으로 전용 pool을 선택하도록 수정. live HPA/PDB 1, 세 앱 1 Pod 상태에서 Managed 단일 노드로 재배치. |
| 2026-08-25 | pending | 사용자 승인으로 비용보다 성능 우선: 시작 Managed 1대 공유는 유지하되 default/stress NodePool ceiling을 각각 8 CPU(4×t3.medium), consolidateAfter 10m으로 열고 user/product HPA Max 40 적용. observer는 30초 판단, warm min 4/4/4, HPA Max 최대 2배, NodePool 포화 시 단계 증가 대신 최대 4대 ceiling 즉시 개방, HPA scale-down 600초/유휴 복구 600초로 변경해 spike 사이 capacity를 유지. |
