...
| 2026-08-21 | pending | 탄력 운영 정책 적용 — idle HPA min 2/2/1, user/stress 조기 trigger 18%/35%, worker hard ceiling 6(managed1+default1+stress4), Karpenter consolidateAfter 1m. shared user/product의 preferred hostname spread가 저부하 default 노드를 불필요하게 유지하므로 제거하고 dedicated stress spread는 유지. 무부하에서 실제 2노드와 앱 Ready 확인. |
