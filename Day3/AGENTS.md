...
| 2026-08-22 | pending | profile 시작 지연 개선: no-traffic cooldown에서 Ready 노드는 이미 floor인데 HPA만 60초 이상 잔류하면 `tune.ps1`이 scaleDown을 임시 0s/100%로 가속하고 min 도달 즉시 표준 300s/Min 정책을 복구. 이전 6~10분 stair-step 대기와 cooldown timeout 오염을 제거. user25 near-ceiling 증거로 다음 seed user target33→28 적용. parse/self-test 36/36·terraform validate 통과. |
