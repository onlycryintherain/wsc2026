...
| 2026-08-25 | pending | measured tuner 첫 live 실행이 BASE에서 stress NodePool CPU 8/4노드 hard limit(`8 Ready + 4 Pending`, `all available instance types exceed limits`)을 확인했는데도 FINAL 3프로필을 반복한 lifecycle 버그 수정. explicit limit을 `NODEPOOL_LIMIT`로 분류하고 terminal infrastructure/generator stop 시 FINAL을 생략하며, 20분은 최소가 아닌 hard ceiling임을 시작 로그/README에 명시. self-test 23/23·localhost 4/4 통과. |
